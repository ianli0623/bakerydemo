...
import type { BakeryContentLink, BakeryImage } from '../shared/types/bakery.ts';
...
test('getEmbedPresentation converts supported video URLs to embed players', () => {
  ...
  assert.deepEqual(getEmbedPresentation('https://vimeo.com/76979871'), {
    kind: 'iframe',
...
test('getEmbedPresentation falls back to a safe link for other providers', () => {
  assert.deepEqual(getEmbedPresentation('https://example.com/media/123'), {
    kind: 'link',
